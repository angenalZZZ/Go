...
func NewClient(opentsdbCfg config.OpenTSDBConfig) (Client, error) {
	opentsdbCfg.Host = strings.TrimSpace(opentsdbCfg.Host)
	if len(opentsdbCfg.Host) <= 0 {
	...
	tsdbEndpoint := fmt.Sprintf("http://%s", opentsdbCfg.Host)
...
func (c *clientImpl) Ping() error {
	conn, err := net.DialTimeout("tcp", c.opentsdbCfg.Host, DefaultDialTimeout)
	if err != nil {
